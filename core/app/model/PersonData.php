...
class PersonData {
    ...
    public function PersonData(){
        ...
        $this->image = "";
        $this->password = "";
         $this->gpsimei = ""; // Nuevo campo gpsimei
        $this->gpstel = ""; // Nuevo campo gpstel
        $this->created_at = "NOW()";
...
public function add_client(){
    $sql = "INSERT INTO person (no, name, lastname, address1, email1, phone1, is_active_access, password, kind, credit_limit, has_credit, gpsimei, gpstel, created_at) ";
    $sql .= "VALUES (\"$this->no\", \"$this->name\", \"$this->lastname\", \"$this->address1\", \"$this->email1\", \"$this->phone1\", \"$this->is_active_access\", \"$this->password\", 1, \"$this->credit_limit\", $this->has_credit, \"$this->gpsimei\", \"$this->gpstel\", NOW())";
    Executor::doit($sql);
}
    ...
    $sql = "UPDATE ".self::$tablename." SET 
            ...
            phone1=\"$this->phone1\",
            is_active_access=\"$this->is_active_access\",
            password=\"$this->password\",
            has_credit=\"$this->has_credit\",
            credit_limit=\"$this->credit_limit\",
            gpsimei=\"$this->gpsimei\",
            gpstel=\"$this->gpstel\"
            WHERE id=$this->id";
    Executor::doit($sql);
}
...
// partiendo de que ya tenemos creado un objecto PersonData previamente utilizamos el contexto
    public function update(){
        $sql = "update ".self::$tablename." set name=\"$this->name\",email1=\"$this->email1\",address1=\"$this->address1\",lastname=\"$this->lastname\",phone1=\"$this->phone1\",gpsimei=\"$this->gpsimei\",gpstel=\"$this->gpstel\" where id=$this->id";
        Executor::doit($sql);
    }
