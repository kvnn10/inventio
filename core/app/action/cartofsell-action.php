...
<?php if(isset($_SESSION["errors"])): ?>
    ...
    <table class="table table-bordered table-hover">
        ...
        <?php foreach ($_SESSION["errors"] as $error): ?>
            <?php $product = ProductData::getById($error["product_id"]); ?>
            <tr class="danger">
            ...
            <div class="box box-primary">
                <table class="table table-bordered table-hover">
                    ...
                    <?php foreach($_SESSION["cart"] as $p): ?>
                        ...
                        if($product != null) {
                            $price = isset($product->price_out) ? $product->price_out : 0;
                            $px = PriceData::getByPS($product->id, StockData::getPrincipal()->id);
                            if($px != null){ $price = $px->price_out; }
                            ?>
                            <tr>
                                <td><?php echo isset($product->code) ? $product->code : 'N/A'; ?></td>
                                <td><?php echo isset($p["q"]) ? $p["q"] : 'N/A'; ?></td>
                                <td><?php echo isset($product->unit) ? $product->unit : 'N/A'; ?></td>
                                <td><?php echo isset($product->name) ? $product->name : 'N/A'; ?></td>
                                <td><b><?php echo $symbol; ?> <?php echo number_format($price, 2, ".", ","); ?></b></td>
                                <td><b><?php
                                        $pt = $price * $p["q"];
                                        $total += $pt;
                                        echo $symbol . " " . number_format($pt, 2, ".", ",");
                                    ?></b></td>
                                <td style="width:30px;">
                                    <a id="clearcart-<?php echo isset($product->id) ? $product->id : ''; ?>" class="btn btn-xs btn-danger"><i class="glyphicon glyphicon-remove"></i> Quitar</a>
                                    <script>
